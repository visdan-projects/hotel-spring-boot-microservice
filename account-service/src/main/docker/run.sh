...
echo "========> Engaging Account Service for profile -> $PROFILE via discovery -> $DISCOVERY_SERVICE_URI:$SERVER_PORT"
java -Djava.security.egd=file:/dev/./urandom -Dserver.port=$SERVER_PORT -Deureka.client.serviceUrl.defaultZone=$DISCOVERY_SERVICE_URI -Dspring.cloud.config.uri=$CONFIG_SERVICE_URI -Dspring.zipkin.baseUrl=$TRACING_SERVICE_URI -Dsecurity.oauth2.resource.userInfoUri=$AUTH_SERVICE_URI -Dspring.profiles.active=$PROFILE -jar /usr/local/accountservice/@project.build.finalName@.jar